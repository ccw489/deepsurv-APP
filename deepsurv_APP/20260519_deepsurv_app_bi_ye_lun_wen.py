...
def classify_risk(log_risk, train_stats):
    percentiles = train_stats.get("percentiles", {})
    med = percentiles.get(50, percentiles.get("50", 0.0))
...
def pretty_name(feat: str) -> str:
    return DISPLAY_NAMES.get(feat, feat)
...
def make_export_figure(expl, risk_label: str, risk_plain_text: str) -> plt.Figure:
    ...
    gs = fig.add_gridspec(3, 1, height_ratios=[0.8, 6.0, 2.0])
...
def main():
    ...
    if run:
        ...
        expl = shap.Explanation(
            ...
            data=x_np[0],
            feature_names=[pretty_name(f) for f in feature_names],
        )

        # ---- On-screen SHAP：右侧卡片内只放图，标题行在上面已对齐 ----
        plt.close("all")
        fig_inline = plt.figure(figsize=(6, 3.2))
